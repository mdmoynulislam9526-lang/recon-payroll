...
sig_base64_str = ""
sig_html_element = (
    f"<img src='data:image/png;base64,{sig_base64_str}' style='width: 150px;'>"
    if sig_base64_str
...
with col1:
    ...
    with st.form("employee_form", clear_on_submit=True):
        ...
        category = st.selectbox(
            "Select Category",
            ["Manager", "Officer", "Worker (Permanent)", "Worker (Daily Basis)"],
        )
    ...
    with st.container():
        ...
        with col_act2:
            if st.button("Delete ❌", key=f"{prefix}_del_{eid}", type="secondary"):
                # Supabase Delete
                supabase.table("employees_final_version").delete().eq(
                    "emp_id", eid
                ).execute()
                supabase.table("monthly_attendance_records").delete().eq(
                    "emp_id", eid
                ).execute()
                st.rerun()
            ...
            with st.form(key=f"form_{prefix}_{eid}"):
                ...
                if st.form_submit_button("Save"):
                    ...
                    supabase.table("employees_final_version").update(
                        {"name": ch_name, "salary": float(ch_salary)}
    ...
    if rows:
        ...
        with c_col2:
            select_y = st.selectbox(
                "Select Year",
                available_years,
                index=available_years.index(str(current_year)),
            )
        ...
        response_att = (
            supabase.table("monthly_attendance_records")
            .select("*")
            .eq("month_year", full_month)
            .execute()
        )
        ...
        # টোটাল হিসাব
        total_payout, total_bonus, total_ot, total_deductions, total_advance = (
            0.0,
            0.0,
            0.0,
            0.0,
        ...
        # --- Tabs ---
        tab_emp, tab0, tab1, tab2 = st.tabs(
            [
                "👥 All Employees",
                "🔍 Search Employee",
                "📄 Individual Pay Slip",
                "📊 Attendance & Payroll Processor",
            ]
        )
        ...
        with tab0:
            ...
            if search_query:
                ...
                search_results = [
                    r
                    for r in rows
                    if search_query.lower() in str(r["emp_id"]).lower()
...
with tab1:
    pay_search = st.text_input(
        "Enter Employee ID or Name for Pay Slip", key="pay_slip_search_input"
    )
    if pay_search:
        pay_results = [
            r
...
pdf_emp_data = (
    selected_emp["emp_id"],
    selected_emp["name"],
    selected_emp["designation"],
    selected_emp["category"],
    selected_emp["department"],
    house_rent,
    medical,
    adv_paid,
    net_final,
...
st.download_button(
    ...
    file_name=f"PaySlip_{selected_emp['emp_id']}.pdf",
        ...
        with st.form("bulk_sheet_form_v5"):
            ...
            for r in filtered_rows:
                ...
                with col_in1:
                    ...
                    total_target_days = st.number_input(
                        "Total Target Days",
                        1,
                        100,
                        target_val,
                        key=f"target_{r['emp_id']}",
                    )
                    a_d = st.number_input(
                        "Absent Days",
                        0,
            ...
            confirm_save = st.checkbox(
                f"I intentionally want to save/overwrite data for **{full_month}**."
            )

            # বাটনটি এখন ফর্মের ভেতরে আছে (এটিই সঠিক ইন্ডেন্টেশন)
            if st.form_submit_button(
                "💾 Save Entry to Database", use_container_width=True, type="primary"
            ):
                if not confirm_save:
...
display_titles = [
    "💼 Managers Summary",
    "👔 Officers Summary",
    "🛠️ Workers (Permanent) Summary",
    "📆 Workers (Daily Basis) Summary",
]
